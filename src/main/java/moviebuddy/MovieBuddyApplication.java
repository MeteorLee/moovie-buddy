...
public class MovieBuddyApplication {
     ...
     * 애플리케이션 추가 요구사항:
     * 
     * (완료) XML 문서로 작성된 영화 메타데이터도 다룰 수 있게 기능을 확장하라
     * (완료) 영화 메타데이터 위치를 변경할 수 있도록 하라
     * (완료) 영화 메타데이터 읽기 속도를 빠르게 하라
     * (완료) 시스템 언어설정에 따라 애플리케이션 메시지가 영어 또는 한글로 출력되게 하라
     */
...
//        final Environment environment = applicationContext.getEnvironment();
        ...
        commandActions.put(Command.Quit, arguments -> {
            output.println(messageSource.getMessage("application.commands.quit", new Object[]{}, Locale.getDefault()));
            running.set(false);
        });
        ...
        commandActions.put(Command.DirectedBy, arguments -> {
            ...
            moviesDirectedBy.forEach(it -> {
                String data = String.format(messageSource.getMessage("application.commands.directedBy.format", new Object[]{}, Locale.getDefault()),
                        counter.getAndIncrement(), it.getTitle(), it.getReleaseYear(), it.getDirector(), it.getWatchedDate().format(Movie.DEFAULT_WATCHED_DATE_FORMATTER));
                output.println(data);
            });
...
//                }
                ...
                // application.errors.UndefinedCommandActionException
